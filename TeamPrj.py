...
asian_food = ['쇠고기쌀국수', '얼큰 쇠고기 칼국수', '베트남쌀국수', '허기진 내영혼의 쌀국수', '얼큰 쌀국수', '베트남볶음면', '닭고기쌀국수', '쇠고기세트', '베트남세트', '왕새우세트', '치즈볼세트', '덕성여대 스페셜 세트1', '덕성여대 스페셜 세트2', '베트남새우볶음밥', '계란후라이볶음밥', '파인애플볶음밥', '왕새우튀김', '치즈볼', '베트남군만두짜조', '하이퐁탕수육', '버팔로 윙/봉']
ramen = ['미스터라멘', '돈코츠라멘', '시오라멘', '소유라멘', '탄탄멘', '매운돈코츠라멘', '마라탕멘', '열라맵닭라멘', '해장김치라멘', '스끼야끼라멘', '바질라멘', '공기밥', '교자튀김', '치킨가라아게', '버팔로윙']
japanese_western_food = ['돈까스우동세트', '제주흑돼지돈까스', '제주흑돼지돈까스정식', '통모짜돈까스정식', '카레밥', '제주흑돼지돈까스카레', '새우후라이 카레', '가라아게 카레', '돈부리', '믹스동', '돈코츠라멘', '우동', '카레우동', '탄탄면', '냉모밀', '판모밀']
korean_food = ['우삼겹샐러드', '리코타샐러드', '그린샐러드', '닭가슴샐러드', '돼지김치찌개', '부대찌개', '된장찌개', '차돌된장찌개', '불닭마요덮밥', '참치마요덮밥', '제육덮밥', '오징어덮밥', '야채비빔밥', '소불고기덮밥', '불닭덮밥']
snack_food = ['크앙-라떡', '매콤크림파스타떡볶이', '크앙-라떡토핑튀김', '크앙-라떡토핑고기', '라면', '삼쫄', '호화스런 짜계치', '직화삼겹덮밥', '직화불삼겹덮밥', '직화우삼겹덮밥', '직화안창살덮밥', '모듬튀김', '크앙김말이튀김', '롱순대꼬치튀김', '치즈범벅-후라이']
...
def prepared(category, category_prepared, category_remaining):
    print()
    print('준비된 메뉴의 수량을 입력하세요.')
    for i in range(len(category)):
        prepared = int(input(category[i]))
        category_prepared.append(prepared)
        category_remaining.append(prepared)
    print('준비된 메뉴의 수량이 모두 입력되었습니다.')

# 3. 주문된 메뉴의 수량을 입력받는 함수

def order(category, category_prepared, category_remaining):
...
def menu_list(category, category_prepared, category_remaining):
    menu_list_category = []
    ...
    if len(category_prepared) == 0:
        ...
        for i in range(len(category)):
            if category_remaining[i] > 0:
                menu_list_category.append(category[i])
        ...
        for m in range(len(category)):
            if category_remaining[m] <= 0:
                sold_out_category.append(category[m])
